...
<body>
   ...
   <a href="index.php">Home</a>
    ...
    // Check if $sector is an array before using array_slice
    if (is_array($sector)) {
        // Lấy dữ liệu từ chỉ mục thứ 3 trở đi
        $sectorDataSlice = array_slice($sector, 2); // Bỏ 2 chỉ mục đầu tiên, bắt đầu từ chỉ mục 3
        echo "<tr>";
        echo "<td>" . $index++ . "</td>";

        // Hiển thị các giá trị từ chỉ mục 3 trở đi
        foreach ($sectorDataSlice as $value) {
            // Kiểm tra xem $value có phải là mảng hay không
            if (is_array($value)) {
                // Nếu là mảng, bạn có thể hiển thị các số 1, 2, 3, 4 với các liên kết tương ứng
                echo "<td>";
                foreach ($value as $i => $link) {
                    // Mỗi phần tử trong mảng $value sẽ là một liên kết
                    // Thêm target="_blank" để mở cửa sổ mới khi nhấp vào
                    echo "<a href='" . htmlspecialchars($link) . "' target='_blank'>" . ($i + 1) . "</a> "; 
                }
                echo "</td>";
            } else {
                // Nếu không phải là mảng (chắc là chuỗi), hiển thị bình thường
                echo "<td>" . htmlspecialchars($value) . "</td>";
            }
        }
        // Thêm cột Action với nút "Update"
        echo "<td><button onclick='window.location.href=\"update_page.php?id=" . $sector['sector_name'] . "\"'>Update</button></td>";

        echo "</tr>";
    } else {
        // If $sector is not an array, you can handle it differently or skip it
